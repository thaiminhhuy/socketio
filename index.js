...
io.on('connection', (socket) => {
  ...
  socket.on("ClientWeb", (data)=> {
   parseInt()
    if(data.type === "Red")
    { 
      console.log("Red",parseInt(data.value) );
      io.sockets.emit("ClientLedRed",parseInt(data.value) - 4)
    } 
    if(data.type === "Green"){
      console.log("Green", parseInt(data.value));
      io.sockets.emit("ClientLedGreen",parseInt(data.value))
    }
